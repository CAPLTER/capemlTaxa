...
#' checks for duplicate matches, and writes the result to \code{taxa_map.csv}.
#'
...
#' \code{dplyr} for data manipulation.
#'
#' @importFrom dplyr mutate left_join arrange select row_number filter distinct across all_of
...
write_taxa_map <- function(
  ...
  taxa_ids <- taxa_df |>
    ...
    dplyr::distinct(dplyr::across(dplyr::all_of("taxa_clean"))) |>
  ...
  taxonomy_itis_duplicates <- taxonomy_itis |>
    dplyr::mutate(duplicate = duplicated(.data$index)) |>
...
get_itis_data <- function() {
  ...
  taxadb_itis_tbl_query <- glue::glue_sql(
    "
    SELECT *
    FROM { get_taxadb_tbl_name(connection = db) }
    ;
    ",
    .con = DBI::ANSI()
  )
